...
marginals_dict = fda.KeyDependentDictForMarginals()
...
# print(len(np.zeros((12,1),dtype=float)))

import time

bits_of_interest = [0, 1, 2, 9, 12,20,22]
bitstring_global = '0101010101001010101001010101110'

t0 = time.time()
s = ''
for qubit_index in bits_of_interest:
    s += bitstring_global[qubit_index]
t1 = time.time()
s2 = ''.join([bitstring_global[qubit_index] for qubit_index in bits_of_interest])
t2 = time.time()
